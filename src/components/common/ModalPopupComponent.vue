  ...
  <v-dialog v-model="dialog" max-width="500" persistent>
    <v-card class="rounded-dialog" rounded="xl">
      ...
      <v-card-title class="d-flex align-center">
        <span>{{ title }}</span>
        <v-spacer></v-spacer> <!-- 왼쪽에 공간을 채워서 X 버튼을 오른쪽 끝으로 밀어냄 -->
        <v-btn icon=mdi-close @click="cancel" variant="none">
        </v-btn>
      </v-card-title>
    ...
    <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="blue-darken-1" variant="outlined" @click="confirm">
          확인
        </v-btn>
        <v-btn color="red-darken-1" variant="outlined" @click="cancel">
          취소
        </v-btn>
      </v-card-actions>
    </v-card>
